...
import numpy as np
import torch
from video_utils.backgroundremover.backgroundremover.bg import DEVICE, Net, iter_frames, remove_many
import shlex
...
def transparentvideooverimage(output, overlay, file_path,
    ...
    cmd = "nice -10 ffmpeg -y -i %s -i %s -filter_complex 'scale2ref[img][vid];[img]setsar=1;[vid]nullsink' -q:v 2 %s" % (
