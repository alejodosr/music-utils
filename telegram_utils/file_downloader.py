...
TELEGRAM_BOT_TOKEN = getenv("TELEGRAM_BOT_TOKEN", "5358961967:AAFPKENLkYcXVVCLoSOifW5DinPXBAAJOtI")  # <-- Configure here
TELEGRAM_CHAT_ID = getenv("TELEGRAM_CHAT_ID", "5358961967")  # <-- Configure here
TELEGRAM_REFRESH_SECONDS = getenv("TELEGRAM_REFRESH_SECONDS", 1)
DOWNLOADS_FOLDER = getenv("DOWNLOADS_FOLDER", "/home/alejandro/Downloads")
    ...
    while not user_quit:
        ...
        for update in telegram_updates:
            ...
            if user_command and user_command.lower() == "quit":
                ...
                user_quit = True
                telegram_updates = bot.get_updates(offset=update_id,
                                                   timeout=TELEGRAM_TIMEOUT)  # mark this as read or Telegram will send it again
                break
            ...
            try:
                ...
                bot.send_message(chat_id=TELEGRAM_CHAT_ID,
                                 text="Downloading file %s (%i bytes)" % (newfile.file_name, newfile.file_size))
            ...
            try:
                ...
                if len(photos) > 0:
                    ...
                    bot.send_message(chat_id=TELEGRAM_CHAT_ID,
                                     text="Downloading photo %s (%i bytes)" % (name, photo.file_size))
            ...
            try:
                ...
                bot.send_message(chat_id=TELEGRAM_CHAT_ID,
                                 text="Downloading audio %s (%i bytes)" % (name, audio.file_size))
            ...
            try:
                ...
                bot.send_message(chat_id=TELEGRAM_CHAT_ID,
                                 text="Downloading voice %s (%i bytes)" % (name, voice.file_size))
